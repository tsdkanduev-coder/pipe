...
struct LLMProviderSetupView: View {
    ...
    case .modelDownload(let command):
      VStack(alignment: .leading, spacing: 24) {
        VStack(alignment: .leading, spacing: 8) {
          ...
          Text("This model enables Sled to understand what's on your screen")
            .font(.custom("Figtree", size: 14))
            .foregroundColor(.black.opacity(0.6))
        }
  ...
  func saveConfiguration() -> Bool {
    ...
    if providerType == .openAICompatible {
      guard persistOpenAICompatibleSettings() else {
        setupState.saveErrorMessage =
          "Sled couldn't save this endpoint configuration. Your previous configuration is still active."
        return false
      }
    }
    ...
    do {
      ...
      setupState.saveErrorMessage =
        "Sled couldn't finish saving this provider. Please try again."
      return false
    }
  }
  ...
  func completeSetup() {
    ...
    guard onComplete() else {
      setupState.saveErrorMessage =
        "The provider is configured, but Sled couldn't update your routing. Your previous selection is still active."
      return
    }
  }
